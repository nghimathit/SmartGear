...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!in_array(strtolower($type_file), $type_fileAllow)) {
        $error['file_1'] = "The file you just selected is not supported by the system, please select an image";
    }
    //Check file size
    $size_file = $_FILES['file_1']['size'];
    if ($size_file > 5242880) {
        $error['file_1'] = "The file you selected cannot exceed 5MB";
    }
// Check if the file exists on the system
    if (file_exists($target_file)) {
        $error['file_1'] = "The file you selected already exists on the system";
    }
//
    if (empty($error)) {
